...
const REFUND_ORDER = `
  mutation RefundOrder($order_id: uuid!) {
    update_orders_by_pk(
      pk_columns: { order_id: $order_id }
      _set: { status: "refunded", payment_status: "refunded", updated_at: "now()" }
    ) {
      order_id
      status
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!customer) {
      return NextResponse.json(
        { success: false, error: 'Please sign in to request a refund.' },
        { status: 401 },
      );
    }
    ...
    try {
      ...
      console.error('[Menu Orders] Refund email failed:', emailErr);
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('[Menu Orders] Refund order error:', error);
    return NextResponse.json(
