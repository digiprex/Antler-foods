...
interface PopupConfigQueryResponse {
  templates: Array<{
    category: string;
    config: any;
    created_at: string;
...
export async function GET(request: Request) {
  try {
    ...
    const data = await graphqlRequest<PopupConfigQueryResponse>(GET_POPUP_CONFIG, { restaurant_id: restaurantId });
...
export async function POST(request: Request) {
  try {
    ...
    const currentData = await graphqlRequest<PopupConfigQueryResponse>(GET_POPUP_CONFIG, { restaurant_id: restaurantId });
