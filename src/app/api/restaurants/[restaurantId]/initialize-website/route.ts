...
// Initialize Vertex AI client (lazy — only used when both Bedrock and OpenAI fail)
...
function getVertexAIModel() {
  ...
  return vertexAI.getGenerativeModel({ model: process.env.VERTEX_AI_MODEL || 'gemini-2.5-pro' });
}
...
/**
 * Helper function to generate content using Vertex AI (Gemini) as final fallback
 */
async function generateContentWithVertexAI(prompt: string, maxTokens: number = 1000) {
  try {
    if (!process.env.GOOGLE_CLOUD_PROJECT_ID) {
      console.log('Google Cloud Project ID not configured, skipping Vertex AI fallback');
      return null;
    }
    ...
    try {
      ...
      try {
        generatedContent = JSON.parse(cleanedJson);
        console.log('✅ Successfully parsed cleaned JSON for Vertex AI fallback');
      } catch {
        console.error('Failed to parse Vertex AI JSON response');
        throw new Error('Could not parse JSON from Vertex AI response');
      }
    }
    console.log('✅ Generated content using Vertex AI fallback');
    return generatedContent;
