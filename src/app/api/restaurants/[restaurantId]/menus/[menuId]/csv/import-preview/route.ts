...
import { previewMenuCsvImport } from '@/lib/server/menu-csv';
...
export async function POST(
  ...
  try {
    ...
    const preview = await previewMenuCsvImport({
      restaurantId,
      menuId,
      csvText,
      fileName,
    ...
    return NextResponse.json(
      {
        success: false,
        error: resolveMenuCsvErrorMessage(
          error,
          'Failed to preview menu CSV import.',
        ),
      },
      { status: 500 },
    );
