...
export async function GET(
  ...
  try {
    ...
    const syncRequested = request.nextUrl.searchParams.get('sync') !== 'false';
    const account = syncRequested
      ? await syncRestaurantStripeAccountByRestaurantId(restaurantId)
      : await getRestaurantStripeAccountByRestaurantId(restaurantId);
    ...
    return handleRouteError(caughtError, 'Failed to load Stripe account status.');
  }
}
