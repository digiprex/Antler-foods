  ...
  try {
    ...
    try {
      ...
      if ((domainData as any).restaurants_by_pk) {
        ...
        console.log('[Review Config POST] Custom domain check:', hasCustomDomain ? 'EXISTS (production mode)' : 'NOT EXISTS (staging mode)');
      }
    } catch (error) {
      console.error('[Review Config POST] Error checking custom domain:', error);
      ...
      if (templateId) {
        ...
        if ((templateStatus as any).templates_by_pk) {
          ...
          if (!template.is_deleted && !template.is_published) {
            ...
            console.log('[Review Config POST] Section is draft - will mark as deleted and create new record');
            ...
            console.log('[Review Config POST] Section is published with custom domain (PRODUCTION) - will keep published record and create draft');
          } else if (!template.is_deleted && template.is_published && !hasCustomDomain) {
            // Case 3: is_deleted: false & is_published: true but NO custom domain
            shouldMarkAsDeletedAndCreateNew = true;
            console.log('[Review Config POST] Section is published without custom domain (STAGING) - will mark as deleted and create new record');
          }
        }
        ...
        const currentData = pageId
          ...
          : await graphqlRequest(GET_REVIEW_CONFIG, {
              restaurant_id: restaurantId,
            });
    ...
    console.log('[Review Config POST] Saving review with page_id:', pageId);
    ...
    if (!isNewSection && existingOrderIndex !== null && existingOrderIndex !== undefined) {
      ...
      console.log('[Review Config POST] Preserving existing order_index:', orderIndex);
    } else if (pageId) {
      try {
        ...
        if (isNewSection) {
          ...
          console.log('[Review Config POST] New section order_index:', orderIndex, '(max was:', maxOrder, ')');
        } else {
          // For updates to existing sections, use max order or 0
          orderIndex = maxOrder !== null && maxOrder !== undefined ? maxOrder : 0;
          console.log('[Review Config POST] Update section order_index:', orderIndex);
        }
      } catch (error) {
        console.error('[Review Config POST] Error fetching max order_index:', error);
    ...
    console.log('[Review Config POST] Creating new template with is_published:', isPublished, 'ref_template_id:', refTemplateIdToUse,
