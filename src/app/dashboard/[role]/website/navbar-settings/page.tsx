...
function NavbarSettingsContent() {
  ...
  const checkStagingDomain = async () => {
    ...
    try {
      ...
      console.log('Navbar Settings - API Response Status:', res.ok, res.status);
      ...
      console.log('Navbar Settings - API Response Data:', data);

      if (!data.success || !data.data) {
        console.log('Navbar Settings - No success or no data');
        setStagingDomainExists(false);
        return;
      }

      const hasStagingDomain = Boolean(data.data.staging_domain?.trim());
      console.log('Navbar Settings - Staging Domain:', data.data.staging_domain);
      console.log('Navbar Settings - Has Staging Domain:', hasStagingDomain);
      ...
      <div className="flex min-h-[400px] items-center justify-center p-6">
        <div className="max-w-md text-center">
          ...
          <p className="mb-6 text-sm text-gray-600">
            This restaurant doesn't have a website created yet. You need to create a site first before configuring navbar settings.
          </p>
          <button
            onClick={() => {
  ...
  // Show form only if staging domain exists
  if (restaurantId && restaurantName && stagingDomainExists === true) {
    return <NavbarSettingsForm />;
  }

  // Default: no restaurant selected
  return (
