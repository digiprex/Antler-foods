...
export function getShapeRadius(shape: CustomSectionMediaShape | undefined, fallback = '1.5rem') {
  switch (shape) {
    case 'circle':
      return '999px';
    case 'arched':
...
export function CustomSectionMedia({
  ...
  return (
    <div className={`relative overflow-hidden bg-slate-100 ${className}`} style={style}>
      {image?.url ? (
        <img src={image.url} alt={image.alt || label} className={`h-full w-full ${fit === 'contain' ? 'object-contain' : 'object-cover'}`} />
      ) : (
        <div
...
function Button({
  ...
  const backgroundColor =
    button.bgColor ||
    (variant === 'outline' || variant === 'ghost' ? 'transparent' : fallbackStyle.backgroundColor);
  const color =
    button.textColor ||
    (variant === 'outline' || variant === 'ghost' ? fallbackStyle.borderColor : fallbackStyle.color);
  ...
  return (
    <a
      ...
      style={{
        backgroundColor,
        color,
        border: variant === 'ghost' ? '1px solid transparent' : `1px solid ${borderColor}`,
      }}
    >
      {button.label}
    </a>
  ...
  return (
    <div className={`mt-6 flex flex-wrap gap-3 ${justify}`}>
      ...
      {secondaryButton?.label ? (
        <Button button={secondaryButton} fallbackStyle={buttonStyles.secondary} />
      ) : null}
    </div>
  );
}
...
export function CustomSectionIntro({
  ...
  return (
    <div className={`flex flex-col ${alignment}`} style={{ maxWidth: maxWidth || '100%' }}>
      {config.badgeText ? (
        <span className="mb-3 inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em]" style={badgeStyle}>
          {config.badgeText}
        </span>
      ) : null}
      {config.eyebrow ? (
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.22em]" style={eyebrowStyle}>
          {config.eyebrow}
        </p>
      ) : null}
...
export function CustomSectionItemCard({
  ...
  return (
    <div
      className="flex h-full flex-col gap-4 border p-5"
      style={{
        backgroundColor,
        borderColor,
        borderRadius: radius,
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {item.statValue ? (
          <span className="text-xs font-semibold uppercase tracking-[0.18em]" style={{ color: accentColor }}>
            {item.statValue} {item.statLabel ? <span className="text-slate-500">{item.statLabel}</span> : null}
          </span>
        ) : null}
      </div>
      {item.eyebrow ? (
      ...
      {item.ctaLabel ? (
        <a href={item.ctaHref || '#'} className="mt-auto text-sm font-semibold" style={{ color: accentColor }}>
          {item.ctaLabel}
        </a>
      ) : null}
    </div>
