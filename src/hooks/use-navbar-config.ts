/**
 * Custom React Hook for Navbar Configuration
 * 
 * This hook provides an easy way to fetch and manage navbar configuration
 * from the API. It can be used in any component that needs navbar data.
 * 
 * @example
 * ```tsx
 * const { config, loading, error, refetch } = useNavbarConfig();
 * 
 * if (loading) return <LoadingSkeleton />;
 * if (error) return <div>Error: {error}</div>;
 * 
 * return <Navbar {...config} />;
 * ```
 */

'use client';

import { useState, useEffect, useCallback } from 'react';
import type { NavbarConfig } from '@/types/navbar.types';
import { DEFAULT_NAVBAR_CONFIG } from '@/types/navbar.types';

interface UseNavbarConfigOptions {
  /**
   * API endpoint to fetch configuration from
   * @default '/api/navbar-config'
   */
  apiEndpoint?: string;
  
  /**
   * Whether to fetch on mount
   * @default true
   */
  fetchOnMount?: boolean;
  
  /**
   * Override configuration (skips API call if provided)
   */
  overrideConfig?: Partial<NavbarConfig>;
  
  /**
   * Callback when fetch succeeds
   */
  onSuccess?: (config: NavbarConfig) => void;
  
  /**
   * Callback when fetch fails
   */
  onError?: (error: Error) => void;
}

interface UseNavbarConfigReturn {
  /**
   * The navbar configuration (merged with defaults)
   */
  config: NavbarConfig | null;
  
  /**
   * Whether the configuration is currently being fetched
   */
  loading: boolean;
  
  /**
   * Error message if fetch failed
   */
  error: string | null;
  
  /**
   * Function to manually refetch the configuration
   */
  refetch: () => Promise<void>;
  
  /**
   * Function to update the configuration locally
   */
  updateConfig: (updates: Partial<NavbarConfig>) => void;
}

export function useNavbarConfig(
  options: UseNavbarConfigOptions = {}
): UseNavbarConfigReturn {
  const {
    apiEndpoint = '/api/navbar-config',
    fetchOnMount = true,
    overrideConfig,
    onSuccess,
    onError,
  } = options;

  const [config, setConfig] = useState<NavbarConfig | null>(null);
  const [loading, setLoading] = useState(fetchOnMount && !overrideConfig);
  const [error, setError] = useState<string | null>(null);

  const fetchConfig = useCallback(async () => {
    // If override config is provided, use it directly
    if (overrideConfig) {
      const mergedConfig = { ...DEFAULT_NAVBAR_CONFIG, ...overrideConfig };
      setConfig(mergedConfig);
      setLoading(false);
      onSuccess?.(mergedConfig);
      return;
    }

    setLoading(true);
    setError(null);

    try {
      const response = await fetch(apiEndpoint);

      if (!response.ok) {
        throw new Error(`API returned ${response.status}: ${response.statusText}`);
      }

      const data = await response.json();

      if (data.success && data.data) {
        // Merge with defaults to ensure all required fields are present
        const mergedConfig = { ...DEFAULT_NAVBAR_CONFIG, ...data.data };
        setConfig(mergedConfig);
        onSuccess?.(mergedConfig);
      } else {
        throw new Error(data.error || 'Invalid API response structure');
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      console.error('Failed to fetch navbar config:', err);
      setError(errorMessage);
      
      // Fallback to default configuration
      setConfig(DEFAULT_NAVBAR_CONFIG);
      onError?.(err instanceof Error ? err : new Error(errorMessage));
    } finally {
      setLoading(false);
    }
  }, [apiEndpoint, overrideConfig, onSuccess, onError]);

  const updateConfig = useCallback((updates: Partial<NavbarConfig>) => {
    setConfig(prev => prev ? { ...prev, ...updates } : null);
  }, []);

  useEffect(() => {
    if (fetchOnMount) {
      fetchConfig();
    }
  }, [fetchOnMount, fetchConfig]);

  return {
    config,
    loading,
    error,
    refetch: fetchConfig,
    updateConfig,
  };
}

/**
 * Hook for updating navbar configuration via API
 * 
 * @example
 * ```tsx
 * const { updateNavbar, updating, error } = useUpdateNavbarConfig();
 * 
 * const handleSave = async () => {
 *   await updateNavbar({
 *     restaurantName: "New Name",
 *     ctaButton: { label: "New Button", href: "/new" }
 *   });
 * };
 * ```
 */
export function useUpdateNavbarConfig(apiEndpoint = '/api/navbar-config') {
  const [updating, setUpdating] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const updateNavbar = useCallback(async (updates: Partial<NavbarConfig>) => {
    setUpdating(true);
    setError(null);

    try {
      const response = await fetch(apiEndpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(updates),
      });

      if (!response.ok) {
        throw new Error(`API returned ${response.status}: ${response.statusText}`);
      }

      const data = await response.json();

      if (!data.success) {
        throw new Error(data.error || 'Failed to update navbar configuration');
      }

      return data.data;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Unknown error';
      console.error('Failed to update navbar config:', err);
      setError(errorMessage);
      throw err;
    } finally {
      setUpdating(false);
    }
  }, [apiEndpoint]);

  return {
    updateNavbar,
    updating,
    error,
  };
}
